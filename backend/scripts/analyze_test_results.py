...
print(f"澶辫触鐢ㄤ緥鏁伴噺: {len(failed_tests)}")
...
print("澶辫触鐢ㄤ緥鍒楄〃:")
print("="*80)
for i, test in enumerate(failed_tests[:3], 1):  # 鏄剧ず鍓?涓け璐ョ殑鐢ㄤ緥
    print(f"\n{i}. {test['test_case']}")
    print(f"   鏈嶅姟: {test['service']}")
    print(f"   鎺ュ彛: {test['endpoint']}")
    print(f"   鏂规硶: {test['method']}")
    print(f"   棰勬湡鐘舵€? {test['expected_status']}")
    print(f"   瀹為檯鐘舵€? {test['actual_status']}")
    if 'error' in test:
