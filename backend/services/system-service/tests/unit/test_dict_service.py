...
5. 鑾峰彇瀛楀吀鍒楄〃
6. 缁熻瀛楀吀鏁伴噺
7. 鍒涘缓瀛楀吀椤?8. 鑾峰彇瀛楀吀椤?9. 鏇存柊瀛楀吀椤?10. 鍒犻櫎瀛楀吀椤?"""
...
def mock_dict():
    ...
    dict_obj.name = "鐢ㄦ埛鐘舵€?
    dict_obj.description = "鐢ㄦ埛鐘舵€佸瓧鍏?
...
class TestDictService:
    ...
    def test_create_dict_success(self, dict_service, mock_dict):
        """娴嬭瘯鍒涘缓瀛楀吀鎴愬姛"""
        # 妯℃嫙瀛楀吀绫诲瀷涓嶅瓨鍦?        dict_service.dict_repo.get_by_type = Mock(return_value=None)
        # 妯℃嫙鍒涘缓瀛楀吀
    ...
    def test_create_dict_type_exists(self, dict_service, mock_dict):
        """娴嬭瘯鍒涘缓瀛楀吀锛堢被鍨嬪凡瀛樺湪锛?""
        # 妯℃嫙瀛楀吀绫诲瀷宸插瓨鍦?        dict_service.dict_repo.get_by_type = Mock(return_value=mock_dict)
        
        # 鎵ц鍒涘缓瀛楀吀骞堕獙璇佸紓甯?        dict_data = {
            "type": "existing_type",
            "name": "鐜版湁瀛楀吀"
        }
        with pytest.raises(ValueError, match="瀛楀吀绫诲瀷宸插瓨鍦?):
    ...
    def test_create_dict_item_success(self, dict_service, mock_dict, mock_dict_item):
        ...
        # 妯℃嫙瀛楀吀瀛樺湪
        dict_service.dict_repo.get_by_id = Mock(return_value=mock_dict)
        # 妯℃嫙鍒涘缓瀛楀吀椤?        dict_service.dict_item_repo.create = Mock(return_value=mock_dict_item)
        
        # 鎵ц鍒涘缓瀛楀吀椤?        dict_item_data = {
            "label": "姝ｅ父",
            "value": "1",
            "sort_order": 0
        }
    ...
    def test_get_dict_item_success(self, dict_service, mock_dict_item):
        """娴嬭瘯鑾峰彇瀛楀吀椤规垚鍔?""
        # 妯℃嫙鏌ヨ瀛楀吀椤?        dict_service.dict_item_repo.get_by_id = Mock(return_value=mock_dict_item)
    ...
    def test_get_dict_items_success(self, dict_service, mock_dict_item):
        """娴嬭瘯鑾峰彇瀛楀吀椤瑰垪琛ㄦ垚鍔?""
        # 妯℃嫙鏌ヨ瀛楀吀椤瑰垪琛?        dict_service.dict_item_repo.get_by_dict_id = Mock(return_value=[mock_dict_item])
    ...
    def test_get_dict_items_by_type_success(self, dict_service, mock_dict_item):
        """娴嬭瘯鏍规嵁绫诲瀷鑾峰彇瀛楀吀椤瑰垪琛ㄦ垚鍔?""
        # 妯℃嫙鏌ヨ瀛楀吀椤瑰垪琛?        dict_service.dict_item_repo.get_by_dict_type = Mock(return_value=[mock_dict_item])
    ...
    def test_update_dict_item_success(self, dict_service, mock_dict_item):
        """娴嬭瘯鏇存柊瀛楀吀椤规垚鍔?""
        # 妯℃嫙鏌ヨ瀛楀吀椤?        dict_service.dict_item_repo.get_by_id = Mock(return_value=mock_dict_item)
        # 妯℃嫙鏇存柊瀛楀吀椤?        dict_service.dict_item_repo.update = Mock(return_value=mock_dict_item)
        
        # 鎵ц鏇存柊
        update_data = {"label": "鏇存柊鍚庣殑鏍囩"}
    ...
    def test_delete_dict_item_success(self, dict_service):
        """娴嬭瘯鍒犻櫎瀛楀吀椤规垚鍔?""
        # 妯℃嫙鍒犻櫎瀛楀吀椤?        dict_service.dict_item_repo.delete = Mock(return_value=True)
