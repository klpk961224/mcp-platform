...
class DepartmentService:
    ...
    def create_department(self, dept_data: Dict) -> Department:
        ...
        Args:
            dept_data: 閮ㄩ棬鏁版嵁
                - name: 閮ㄩ棬鍚嶇О锛堝繀濉級
                - code: 閮ㄩ棬缂栫爜锛堝彲閫夛紝鑷姩鐢熸垚锛?                - tenant_id: 绉熸埛ID锛堝繀濉級
                - parent_id: 鐖堕儴闂↖D锛堝彲閫夛級
                - level: 灞傜骇锛堝彲閫夛紝鑷姩璁＄畻锛?                - sort_order: 鎺掑簭锛堝彲閫夛級
        ...
        Raises:
            ValueError: 閮ㄩ棬缂栫爜宸插瓨鍦ㄦ垨閮ㄩ棬鍚嶇О宸插瓨鍦?        """
        logger.info(f"鍒涘缓閮ㄩ棬: name={dept_data.get('name')}, tenant_id={dept_data.get('tenant_id')}")
        
        # 楠岃瘉蹇呭～瀛楁
        if not dept_data.get('name'):
            raise ValueError("閮ㄩ棬鍚嶇О涓嶈兘涓虹┖")
        
        if not dept_data.get('tenant_id'):
            raise ValueError("绉熸埛ID涓嶈兘涓虹┖")
        
        # 妫€鏌ラ儴闂ㄧ紪鐮佹槸鍚﹀瓨鍦?        code = dept_data.get('code')
        if code and self.dept_repo.exists_by_code(code):
            raise ValueError(f"閮ㄩ棬缂栫爜宸插瓨鍦? {code}")
        
        # 妫€鏌ラ儴闂ㄥ悕绉板湪绉熸埛鍐呮槸鍚﹀瓨鍦?        if self.dept_repo.exists_by_name_in_tenant(dept_data['name'], dept_data['tenant_id']):
            raise ValueError(f"閮ㄩ棬鍚嶇О宸插瓨鍦? {dept_data['name']}")
    ...
    def update_department(self, department_id: str, dept_data: Dict) -> Department:
        ...
        """
        logger.info(f"鏇存柊閮ㄩ棬: department_id={department_id}")
        ...
        if dept_data.get('code') and dept_data['code'] != department.code:
            # 妫€鏌ラ儴闂ㄧ紪鐮佹槸鍚﹀瓨鍦?            if self.dept_repo.exists_by_code(dept_data['code']):
                raise ValueError(f"閮ㄩ棬缂栫爜宸插瓨鍦? {dept_data['code']}")
            department.code = dept_data['code']
        
        # 鏇存柊鐖堕儴闂?        if dept_data.get('parent_id') != department.parent_id:
            ...
            # 鏇存柊鎵€鏈夊瓙閮ㄩ棬鐨勫眰绾?            self._update_children_level(department_id)
    ...
    def list_departments(
        ...
        if keyword:
            ...
            total = len(departments)  # TODO: 闇€瑕佷紭鍖栵紝搴旇鍗曠嫭鏌ヨ鎬绘暟
