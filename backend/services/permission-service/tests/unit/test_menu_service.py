...
class TestMenuService:
    ...
    def test_create_menu_success(self, menu_service, mock_menu):
        """娴嬭瘯鍒涘缓鑿滃崟鎴愬姛"""
        # 妯℃嫙鑿滃崟缂栫爜涓嶅瓨鍦?        menu_service.menu_repo.exists_by_code = Mock(return_value=False)
    ...
    def test_create_menu_code_exists(self, menu_service):
        """娴嬭瘯鍒涘缓鑿滃崟锛堢紪鐮佸凡瀛樺湪锛?""
        # 妯℃嫙鑿滃崟缂栫爜宸插瓨鍦?        menu_service.menu_repo.exists_by_code = Mock(return_value=True)
        
        # 鎵ц鍒涘缓鑿滃崟骞堕獙璇佸紓甯?        menu_data = {
            "name": "鐢ㄦ埛绠＄悊",
            "code": "existing_code",
            "tenant_id": "default"
        }
        with pytest.raises(ValueError, match="鑿滃崟缂栫爜宸插瓨鍦?):
            menu_service.create_menu(menu_data)
    
    def test_create_menu_parent_not_found(self, menu_service):
        """娴嬭瘯鍒涘缓鑿滃崟锛堢埗鑿滃崟涓嶅瓨鍦級"""
        # 妯℃嫙鑿滃崟缂栫爜涓嶅瓨鍦?        menu_service.menu_repo.exists_by_code = Mock(return_value=False)
        # 妯℃嫙鐖惰彍鍗曚笉瀛樺湪
    ...
    def test_get_menu_tree_success(self, menu_service, mock_menu):
        """娴嬭瘯鑾峰彇鑿滃崟鏍戞垚鍔?""
        # 妯℃嫙鏌ヨ鑿滃崟鏍?        menu_service.menu_repo.get_tree = Mock(return_value=[mock_menu])
