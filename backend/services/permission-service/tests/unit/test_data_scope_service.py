...
class TestDataScopeService:
    ...
    def test_create_data_scope_success(self, data_scope_service, mock_data_scope):
        """娴嬭瘯鍒涘缓鏁版嵁鑼冨洿鎴愬姛"""
        # 妯℃嫙鏁版嵁鑼冨洿缂栫爜涓嶅瓨鍦?        data_scope_service.data_scope_repo.exists_by_code = Mock(return_value=False)
        # 妯℃嫙鍒涘缓鏁版嵁鑼冨洿
    ...
    def test_create_data_scope_code_exists(self, data_scope_service):
        """娴嬭瘯鍒涘缓鏁版嵁鑼冨洿锛堢紪鐮佸凡瀛樺湪锛?""
        # 妯℃嫙鏁版嵁鑼冨洿缂栫爜宸插瓨鍦?        data_scope_service.data_scope_repo.exists_by_code = Mock(return_value=True)
        
        # 鎵ц鍒涘缓鏁版嵁鑼冨洿骞堕獙璇佸紓甯?        data_scope_data = {
            "name": "鏈儴闂ㄦ暟鎹?,
            "code": "existing_code"
        }
        with pytest.raises(ValueError, match="鏁版嵁鑼冨洿缂栫爜宸插瓨鍦?):
    ...
    def test_check_data_scope_self_true(self, data_scope_service, mock_user_data_scope):
        ...
        # 妯℃嫙鏌ヨ鐢ㄦ埛鏁版嵁鑼冨洿杩斿洖None锛堥粯璁や负self锛?        data_scope_service.user_data_scope_repo.get_by_user_module = Mock(return_value=None)
