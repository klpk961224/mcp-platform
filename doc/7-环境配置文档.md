...
- **最后更新**：2026-01-18
- **文档类型**：环境配置文档
- **当前状态**：后端业务功能开发完成（100%），后端单元测试完成（100%），后端集成测试完成（100%），环境配置文件完善
...
|-----|------|------|
| 认证域服务 | 28001 | 认证授权 |
| 用户域服务 | 28002 | 用户、部门、租户管理 |
| 权限域服务 | 28003 | 角色、权限、菜单管理 |
| 系统域服务 | 28004 | MCP工具、多数据源、字典管理 |
| 支撑域服务 | 28005 | 日志、通知管理 |
| 业务域服务 | 28006 | 工作流管理 |
...
1. **敏感信息**：不要将敏感信息（密码、密钥）提交到代码仓库
2. **环境隔离**：开发、测试、生产环境配置完全隔离
3. **配置验证**：启动应用前验证配置是否正确
4. **配置备份**：定期备份配置文件
5. **配置更新**：配置更新后及时通知相关人员
