...
graph TB
    ...
    subgraph "后端服务层"
        ...
        subgraph "权限域"
            Permission[权限域服务<br/>:28003]
        end
        
        subgraph "系统域"
            System[系统域服务<br/>:28004]
        end
        
        subgraph "支撑域"
            Support[支撑域服务<br/>:28005]
        end
        
        subgraph "业务域"
            Business[业务域服务<br/>:28006<br/>（工作流管理）]
        end
    end
...
| **后端** | 认证域服务 | JWT认证、API Key认证、权限校验、Token管理 | 28001 | MySQL |
| **后端** | 用户域服务 | 用户CRUD、部门管理、租户管理、用户与部门/角色关联 | 28002 | MySQL |
| **后端** | 权限域服务 | 角色管理、权限分配、菜单管理、动态菜单加载 | 28003 | MySQL |
| **后端** | 系统域服务 | MCP工具注册/调用、多数据源管理、字典管理、系统配置 | 28004 | MySQL + PostgreSQL + Oracle |
| **后端** | 支撑域服务 | 登录日志、操作日志、站内信、通知公告 | 28005 | MySQL |
| **后端** | 业务域服务 | 工作流管理（预置审批模板、可视化设计器、审批流程配置、流程监控） | 28006 | MySQL |
...
graph TB
    subgraph "API路由层（api/）"
        API[API路由层<br/>FastAPI Router]
    end
    
    subgraph "业务逻辑层（services/）"
        Service[业务逻辑层<br/>Service类]
    end
    
    subgraph "数据访问层（repositories/）"
        Repository[数据访问层<br/>Repository类]
    end
...
sequenceDiagram
    ...
    participant Auth as 认证域服务<br/>:28001
    participant User as 用户域服务<br/>:28002
...
sequenceDiagram
    ...
    participant APISIX as APISIX网关
    participant Support as 支撑域服务<br/>:28005
    participant MySQL as MySQL数据库
...
sequenceDiagram
    ...
    participant Vue as Vue 3前端
    participant APISIX as APISIX网关
    participant Business as 业务域服务<br/>:28006
    participant MySQL as MySQL数据库
    participant Support as 支撑域服务<br/>:28005
    participant Notify as 通知服务
...
graph TB
    ...
    subgraph "后端服务层"
        subgraph "认证域"
            Auth1[认证域服务1<br/>:28001]
            Auth2[认证域服务2<br/>:28001]
        end
        
        subgraph "用户域"
            User1[用户域服务1<br/>:28002]
            User2[用户域服务2<br/>:28002]
        end
        
        subgraph "权限域"
            Permission1[权限域服务1<br/>:28003]
            Permission2[权限域服务2<br/>:28003]
        end
        
        subgraph "系统域"
            System1[系统域服务1<br/>:28004]
            System2[系统域服务2<br/>:28004]
        end
        
        subgraph "支撑域"
            Support1[支撑域服务1<br/>:28005]
            Support2[支撑域服务2<br/>:28005]
        end
        
        subgraph "业务域"
            Business1[业务域服务1<br/>:28006]
            Business2[业务域服务2<br/>:28006]
        end
    end
